...
TypeScript monorepo (**pnpm** 10): **`packages/*`**, optional **`apps/*`** (e.g. **Starlight** docs in `apps/docs`, **marketing** landing in `apps/landing`), **root** = main Express app. **Turborepo** runs `build` / `typecheck` / `lint` / `clean` / `test` (where defined) across packages; **`pnpm test`** runs `turbo run test` then root `tsx` tests. Shared versions use **`catalog:`** in `pnpm-workspace.yaml`. **TypeScript 6** + **Zod 4** at the repo root and in packages. See `docs/development/development.md` (Monorepo layout).
...
- **Marketing landing (apex /)** is `apps/landing/` (Astro 6 + Tailwind; `build.assets: 'landing'` in `astro.config.mjs` for hashed files). **GitHub Pages** ships one artifact: `pnpm run docs:site:landing` builds Starlight, builds the landing, then `scripts/merge-landing-into-docs-dist.mjs` copies the landing’s `dist/*` to `apps/docs/dist/` (same level as `docs/`), so **https://agent-detective.chapascript.dev/** and **…/docs/** are served from one deploy. **Custom domain** is **GitHub → Settings → Pages** and DNS; see [apps/docs/README.md](apps/docs/README.md).
...
- **Commands (repo root):** `pnpm run docs:site:sync` (sync only), `pnpm run docs:site:dev` (dev server), `pnpm run docs:site` (docs-only production build), `pnpm run docs:site:landing` (docs + landing merged for the same as CI Pages), `pnpm run landing:dev` (landing only), `pnpm docs:config` / `pnpm docs:plugins` (regenerate `docs/reference/generated/*.md`). CI runs `docs:config:check` and `docs:plugins:check`; the GitHub Pages workflow builds Starlight + landing and uploads `apps/docs/dist` on relevant pushes.

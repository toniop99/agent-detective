...
Reshape the post-Fastify HTTP package surface so plugin authors have a clear, self-documenting npm coordinate to depend on, while keeping [`@agent-detective/types`](../../../packages/types/src/index.ts) **type-only**.

Concretely: move all HTTP-related **type declarations** into [`@agent-detective/types`](../../../packages/types/src/index.ts), rename **`packages/core` → `packages/sdk`** (`@agent-detective/core` → `@agent-detective/sdk`) so the runtime helpers are obvious as the plugin SDK, and pull host-only utilities (`applyTagGroups`, tag constants) out of the shared package and into [`src/core/`](../../../src/core/) where only the host imports them.
...
- No change to [`PluginContext`](../../../packages/types/src/index.ts) — `defineRoute` / `registerRoutes` stay as free imports because they are used at module scope, before any context exists.
- No change to the Fastify wiring or Zod-first schema strategy from [`docs/exec-plans/completed/2026-04-http-layer-modernization.md`](../completed/2026-04-http-layer-modernization.md).
...
Add a new [`packages/types/src/http.ts`](../../../packages/types/src/http.ts) (re-exported from [`packages/types/src/index.ts`](../../../packages/types/src/index.ts)) containing the **type declarations only** currently in `packages/core/src/route.ts` and `packages/core/src/spec.ts`:
...
- Add `zod: catalog:` to [`packages/types/package.json`](../../../packages/types/package.json) `dependencies` (it already has `fastify`). Both are used purely for `import type`; the package emits no runtime code.
...
- Move the directory.
- Update [`packages/sdk/package.json`](../../../packages/sdk/package.json) — `"name": "@agent-detective/sdk"`, description "Runtime SDK for agent-detective plugin authors (defineRoute, registerRoutes, zodToPluginSchema)", and trim `dependencies` to what stays in the package (see step 3): `@agent-detective/types`, `fastify`, `fastify-type-provider-zod`, `zod`. Drop `@fastify/swagger` and `openapi-types` — those move with `applyTagGroups` to the host.
- Update [`pnpm-workspace.yaml`](../../../pnpm-workspace.yaml) — no change needed (`packages/*` glob still matches), but verify after rename.
...
Create [`src/core/openapi/tag-groups.ts`](../../../src/core/) with `applyTagGroups()` and the `TagGroup` consumer code, and [`src/core/openapi/tags.ts`](../../../src/core/) with `CORE_PLUGIN_TAG`, `RESERVED_TAGS`, `SCALAR_TAG_GROUPS`, `createTagDescription()`. Both files import the `TagGroup` and `ApplyTagGroupsOptions` *types* from `@agent-detective/types`.

Update [`src/server.ts`](../../../src/server.ts) imports:
...
The host stops depending on `@agent-detective/sdk` for these utilities — it still depends on the SDK for `defineRoute` (used by [`src/core/core-api-controller.ts`](../../../src/core/core-api-controller.ts)).
...
- [`src/server.ts`](../../../src/server.ts) — see step 4.
- [`src/core/core-api-controller.ts`](../../../src/core/core-api-controller.ts) — `defineRoute` from `sdk`, types from `types`.
- [`packages/local-repos-plugin/src/index.ts`](../../../packages/local-repos-plugin/src/index.ts), [`packages/local-repos-plugin/src/presentation/repos-controller.ts`](../../../packages/local-repos-plugin/src/presentation/repos-controller.ts) — same split.
- [`packages/jira-adapter/src/index.ts`](../../../packages/jira-adapter/src/index.ts), [`packages/jira-adapter/src/presentation/jira-webhook-controller.ts`](../../../packages/jira-adapter/src/presentation/jira-webhook-controller.ts) — same split.
- [`packages/pr-pipeline/src/index.ts`](../../../packages/pr-pipeline/src/index.ts) — `zodToPluginSchema` from `sdk`.
- [`test/core/http/route.test.ts`](../../../test/core/http/route.test.ts) — `applyTagGroups` import moves to the host module path; `defineRoute`/`registerRoutes` from `sdk`.
...
- Root [`package.json`](../../../package.json) and [`tsup.config.ts`](../../../tsup.config.ts) — same rename.
...
- [`README.md`](../../../README.md) — update the "Packages" table: rename `@agent-detective/core` row to `@agent-detective/sdk`, description "Runtime SDK for plugin authors (`defineRoute`, `registerRoutes`, `zodToPluginSchema`)". Note types live in `@agent-detective/types`.
- [`AGENTS.md`](../../../AGENTS.md) — update the Packages section with the same rename.
- [`docs/plugins/plugin-development.md`](../../plugins/plugin-development.md) — every `@agent-detective/core` import in the example snippets and `package.json` blocks becomes `@agent-detective/sdk`; route-type imports (when shown) move to `@agent-detective/types`. Update the `pnpm add` example.
- [`docs/architecture/architecture.md`](../../architecture/architecture.md) and [`docs/architecture/adr/0001-layering-and-plugin-boundaries.md`](../../architecture/adr/0001-layering-and-plugin-boundaries.md) — wherever `@agent-detective/core` is named.
- [`docs/development/development.md`](../../development/development.md) — same.
- JSDoc in [`packages/types/src/index.ts`](../../../packages/types/src/index.ts) at `Plugin.register` — update the "Register routes via `registerRoutes(scope, [...])` from `@agent-detective/core`" sentence to point at `@agent-detective/sdk`.
- Run [`pnpm run docs:site:sync`](../../../scripts/sync-starlight-content.mjs) to regenerate the Starlight mirror under [`apps/docs/src/content/docs/`](../../../apps/docs/src/content/docs/).
...
- [ ] [`docs/plugins/plugin-development.md`](../../plugins/plugin-development.md) examples reference `@agent-detective/sdk` and `@agent-detective/types`. Starlight mirror regenerated; `pnpm run lint` (which runs the docs internal-link checker) passes.
- [ ] Changeset under `.changeset/` documents the rename and shows the upgrade diff for plugin authors.

## Notes / decisions

- **Why not put functions on `PluginContext`.** `zodToPluginSchema` is called at *module scope* to populate the `Plugin.schema` literal before any `PluginContext` exists (see [`packages/local-repos-plugin/src/index.ts`](../../../packages/local-repos-plugin/src/index.ts)). `defineRoute` benefits from module-scope usage so plugins can declare routes outside `register()` (the controller-style organisation already used by [`packages/jira-adapter/src/presentation/jira-webhook-controller.ts`](../../../packages/jira-adapter/src/presentation/jira-webhook-controller.ts)). Forcing both through `context` would regress both patterns and add API surface to `PluginContext` for a function (`defineRoute`) whose only purpose is type inference.
...
- [`docs/exec-plans/completed/2026-04-http-layer-modernization.md`](../completed/2026-04-http-layer-modernization.md) — preceding migration that produced the current package shape.
- [`docs/architecture/adr/0001-layering-and-plugin-boundaries.md`](../../architecture/adr/0001-layering-and-plugin-boundaries.md) — layering rules; this plan does not change them, only the package boundaries underneath.
- [`docs/architecture/adr/0002-http-framework.md`](../../architecture/adr/0002-http-framework.md) — Fastify decision that landed `defineRoute` / `registerRoutes` in the first place.
